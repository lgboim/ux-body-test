---
name: ux-body-test
description: Evaluate UI/UX designs through a human-embodied cognition lens — the "Body Test". Use this skill whenever you are creating, reviewing, critiquing, or improving any user interface, whether it's a React component, HTML page, mobile screen, wireframe, Figma screenshot, or any visual design artifact. Also trigger this skill when generating UI code (React, HTML, etc.) to self-evaluate before presenting to the user. The skill simulates how a real human with a physical body, spatial memory, and learned intuitions would experience the interface — something machines fundamentally lack because they only understand "signifiers" (words pointing to words) but not "signifieds" (the real-world objects and sensations those words refer to). This catches usability failures that pass all Material Design / accessibility checks but still feel wrong to humans. Use this even when not explicitly asked — good UX evaluation should be automatic, like a compiler warning.
---
